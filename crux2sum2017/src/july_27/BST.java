...
public class BST {
	...
	public void assques9(int target) {
		assques9(root, root.data + "", target - root.data);
	}
	...
	private void assques9(Node node, String ans, int target) {
		...
		if (node.left != null) {
			assques9(node.left, ans + "->" + node.left.data, target - node.left.data);
		}

		if (node.right != null) {
			assques9(node.right, ans + "->" + node.right.data, target - node.right.data);
		}
