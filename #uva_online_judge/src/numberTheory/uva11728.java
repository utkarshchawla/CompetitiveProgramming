...
public class uva11728 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (true) {
            ...
            int ans = -1;
            for (int i = 0; ; i++) {
                int val = sumDiv(primefactor(i, primes), i);
                if (val > (n + 3000)) break;
                if (val == n) {
                    ans = i;
                }
            }
            fw.println("Case " + tc + ": " + ans);
            tc++;
        }
        fw.close();
    }

    private static int sumDiv(HashMap<Integer, Integer> map, int n) {
        int ans = 1;
        for (int base : map.keySet()) {
            ans *= (Math.pow(base, map.get(base) + 1) - 1) / (base - 1);
        }
        return ans;
